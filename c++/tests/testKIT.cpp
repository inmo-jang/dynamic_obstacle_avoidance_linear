...
int main(int, char*[])
{
	...
	Eigen::Vector3d position_o4(1.15, 3.84, 0);
	...
	auto ptrE1 = std::make_unique<Ellipsoid>(State(position_o1, orientation_o1), 0.2);
	auto ptrE2 = std::make_unique<Ellipsoid>(State(position_o2, orientation_o2), 0.2);
	auto ptrE3 = std::make_unique<Ellipsoid>(State(position_o3, orientation_o3), 0.2);
	auto ptrE4 = std::make_unique<Ellipsoid>(State(position_o4, orientation_o4), 0.2);
	auto ptrE5 = std::make_unique<Ellipsoid>(State(position_o5, orientation_o5), 0.2);
	auto ptrE6 = std::make_unique<Ellipsoid>(State(position_o6, orientation_o6), 0.2);

	ptrE1->set_axis_lengths(Eigen::Array3d(0.05, 0.5, 0));
	ptrE2->set_axis_lengths(Eigen::Array3d(1.5, 0.05, 0));
	ptrE3->set_axis_lengths(Eigen::Array3d(0.05, 2, 0));
	ptrE4->set_axis_lengths(Eigen::Array3d(0.7, 0.1, 0));
