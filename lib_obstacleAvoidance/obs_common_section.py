        ...
        for it_obs2 in range(it_obs1+1,N_obs):
            ...
            if intersection_with_obs1:# Modify intersecition part
                ...
                if True:
                    ...
                    Gamma_temp = ( np.matmul( rotMat[:,:,it_obs2].T , (intersection_sf[it_intersect]-np.tile(obs[it_obs2].x0,(N_inter,1)).T ) )/ np.tile(obs[it_obs2].a,(N_inter,1)).T ) ** (2*np.tile(obs[it_obs2].p,(N_inter,1)).T)
                ...
                if True:
                    ...
                    Gamma_temp = np.matmul( (rotMat[:,:,it_obs1].T ,  (np.array(obs[it_obs2].x_obs_sf).T-np.tile(obs[it_obs1].x0,(N_points,1)).T ) ) / np.tile(obs[it_obs1].a, (N_points,1)).T )
                    ...
                    Gamma_temp = ( np.matmul( rotMat[:,:,it_obs2].T , (np.array(obs[it_obs1].x_obs_sf).T-np.tile(obs[it_obs2].x0,(N_points,1)).T ) ) / np.tile(obs[it_obs2].a, (N_points,1)).T )
                    ...
                    if intersection_sf_temp.shape[1] > 0:
                        ...
                        for kk in range(2):
                            ...
                            for ii in range(1,Gamma_steps):
                                ...
                                Gamma = np.sum( (1/obs[it_obs2_].sf *  np.matmul( rotMat[:,:,it_obs2_].T , (x_obs_sf_interior-np.tile(obs[it_obs2_].x0,(resolution,1)).T ) ) / np.tile(obs[it_obs2_].a, (resolution,1)).T ) ** (2*np.tile(obs[it_obs2_].p, (resolution,1)).T), axis=0)
                            ...
                            if 1 > sum( (1/obs[it_obs2_].sf* np.matmul( rotMat[:,:,it_obs2_].T , ( np.array(obs[it_obs1_].x0) - np.array(obs[it_obs2_].x0) ) )/ np.array(obs[it_obs2_].a) ) ** (2*np.array(obs[it_obs2_].p))):
